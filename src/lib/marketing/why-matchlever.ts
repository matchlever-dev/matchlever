export const WHY_MATCHLEVER = {
  title: "Why MatchLever",
  eyebrow: "Arise Solutions LLC",
  sections: [
    {
      heading: "About Us",
      body: "At Arise Solutions, we believe hiring and job hunting shouldn’t feel like searching for a needle in a haystack. Traditional recruitment is broken—hiring managers spend endless hours sifting through hundreds of unqualified resumes, while talented candidates send their applications into a black hole of automated rejection systems. That's why we created MatchLever.",
    },
    {
      heading: "Our Mission",
      body: "We partner with top-tier companies and highly qualified, vetted job seekers to make finding the right match faster, easier, and smarter.",
      after:
        "By cutting through the noise and focusing on true alignment—skills, culture, and mutual expectations—we eliminate the exhaustion of reviewing hundreds of mismatched applications or endless job boards.",
    },
    {
      heading: "How We Make the Difference",
      bullets: [
        {
          label: "For Job Seekers",
          text: "Skip the frustration of endless blind applications. MatchLever connects you directly with opportunities that match your specific talents, experience, and career goals, saving you time and energy.",
        },
        {
          label: "For Hiring Managers",
          text: "Stop wasting valuable hours scrolling through endless stacks of resumes. MatchLever brings you pre-vetted, high-caliber talent that fits your exact needs right out of the gate.",
        },
        {
          label: "For Everyone",
          text: "A streamlined, efficient process built on quality over quantity, transparency, and getting results quickly.",
        },
      ],
    },
    {
      heading: "Better Matches. Faster Results.",
      body: "At Arise Solutions, our partners are dedicated to respecting your time. Whether you’re looking to take the next step in your career or build a high-performing team, we’re here to help you skip the busywork and get straight to the right match.",
    },
  ],
} as const;
